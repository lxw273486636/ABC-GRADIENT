...
CycleBestParams=Employed(find(ObjEmp==min(ObjEmp),end),:);
CycleWorstParams=Employed(find(ObjEmp==max(ObjEmp),end),:);
...
while ((Cycle <= ABCOpts.MaxCycles))
    ...
    for i=1:ABCOpts.ColonySize/2
        ...
        neighbour1=fix(rand*(ABCOpts.ColonySize/2))+1;
            while(neighbour1==i||neighbour1==neighbour)
                neighbour1=fix(rand*(ABCOpts.ColonySize/2))+1;
            end;
gamma=0.5;
if (Employed(i,Param2Change)==CycleBestParams(Param2Change))
delta=(abs(Employed(neighbour,Param2Change)-Employed(i,Param2Change))+gamma)/2;
Bj=Employed(i,Param2Change)-delta;
Employed2(i,Param2Change)=Employed(i,Param2Change)-delta/2*((Employed(neighbour,Param2Change)-Bj)/(Employed(neighbour,Param2Change)+Bj-Employed(i,Param2Change)))*(rand-0.5)*2;

else if (Employed(i,Param2Change)==CycleWorstParams(Param2Change))
delta=(abs(Employed(neighbour,Param2Change)-Employed(i,Param2Change))+gamma)/2;
Wj=Employed(i,Param2Change)+delta;
Employed2(i,Param2Change)=Employed(i,Param2Change)-delta/2*((Wj-Employed(neighbour,Param2Change))/(Employed(neighbour,Param2Change)+Wj-Employed(i,Param2Change)))*(rand-0.5)*2;

	else if (Employed(neighbour,Param2Change)<=Employed(neighbour1,Param2Change))
Employed2(i,Param2Change)=Employed(i,Param2Change)-(abs(Employed(neighbour,Param2Change)-Employed(i,Param2Change))+abs(Employed(neighbour1,Param2Change)-Employed(i,Param2Change))/4*(Employed(neighbour,Param2Change)-Employed(neighbour1,Param2Change))/(Employed(neighbour,Param2Change)+Employed(neighbour1,Param2Change)-Employed(i,Param2Change)))*(rand-0.5)*2;
	else
Employed2(i,Param2Change)=Employed(i,Param2Change)-(abs(Employed(neighbour,Param2Change)-Employed(i,Param2Change))+abs(Employed(neighbour1,Param2Change)-Employed(i,Param2Change))/4*(Employed(neighbour1,Param2Change)-Employed(neighbour,Param2Change))/(Employed(neighbour,Param2Change)+Employed(neighbour1,Param2Change)-Employed(i,Param2Change)))*(rand-0.5)*2;
	end;
end;
end;
...
%   Gradients provide the search direction      
 ...
 if CycleMin<GlobalMin 
    ...
    %%%Memorize Worst
 CycleWorstIndex=find(FitEmp==min(FitEmp));
 CycleWorstIndex=CycleWorstIndex(end);
 CycleWorstParams=Employed(CycleWorstIndex,:);
