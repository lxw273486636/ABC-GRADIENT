...
while ((Cycle <= ABCOpts.MaxCycles)),
    ...
    for i=1:ABCOpts.ColonySize/2
        ...
        neighbour2=fix(rand*(ABCOpts.ColonySize/2))+1;
            while(neighbour2==i)
                neighbour2=fix(rand*(ABCOpts.ColonySize/2))+1;
            end
neighbour3=1;

while(neighbour3<=ABCOpts.ColonySize/2)
        if(Employed(neighbour3,Param2Change)==GlobalParams(Param2Change))
                neighbour3=neighbour3+1;
        else
            if(abs(Employed(neighbour3,Param2Change)-GlobalParams(Param2Change))<distance)
            distance=abs(Employed(neighbour3,Param2Change)-GlobalParams(Param2Change));
            neighbour4=neighbour3;
            end; 
            neighbour3=neighbour3+1;
        end;
end;
Employed2(i,Param2Change)=GlobalParams(Param2Change)-((SglobalMin(Param2Change)>0)-0.5)*distance*rand+(Employed(i,Param2Change)-Employed(neighbour,Param2Change))*(rand-0.5)*2+(Employed(neighbour1,Param2Change)-Employed(neighbour2,Param2Change))*(rand-0.5)*2;
...
Employed2(i,Param2Change)=GlobalParams(Param2Change)+(Employed(i,Param2Change)-Employed(neighbour,Param2Change))*(rand-0.5)*2+(Employed(neighbour1,Param2Change)-Employed(neighbour2,Param2Change))*(rand-0.5)*2;
