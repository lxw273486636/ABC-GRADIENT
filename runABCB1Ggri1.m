...
while(neighbour1<=ABCOpts.ColonySize/2)
        if(Employed(neighbour1,Param2Change)==GlobalParams(Param2Change))
                neighbour1=neighbour1+1;
        else
            if(abs(Employed(neighbour1,Param2Change)-GlobalParams(Param2Change))<distance)
            distance=abs(Employed(neighbour1,Param2Change)-GlobalParams(Param2Change));
            neighbour2=neighbour1;
            end; 
            neighbour1=neighbour1+1;
        end;
end;
Employed2(i,Param2Change)=GlobalParams(Param2Change)-((SglobalMin(Param2Change)>0)-0.5)*distance*rand+(Employed(i,Param2Change)-Employed(neighbour,Param2Change))*(rand-0.5)*2;
...
Employed2(i,Param2Change)=GlobalParams(Param2Change)-((SglobalMin(Param2Change)>0)-0.5)*distance*rand+(Employed(i,Param2Change)-Employed(neighbour,Param2Change))*(rand-0.5)*2;
%Employed2(i,Param2Change)=Employed(i,Param2Change)-abs(Employed(i,Param2Change)-Employed(neighbour,Param2Change))*rand*2*((Objsen(i,Param2Change)>0)-0.5);
%   Gradients provide the search direction      
         if (Employed2(i,Param2Change)<ABCOpts.lb)
             Employed2(i,Param2Change)=ABCOpts.lb;
         end;
